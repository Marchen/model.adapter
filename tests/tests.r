...
ma.test$set(
	...
	function() {
		class.name <- model.adapter:::get.class.name(private$function.name)
...
ma.test$set(
	...
	function() {
		class.name <- get.class.name(private$function.name)
...
ma.test$set(
	"private", "test__envir",
	function() {
		template <- "Initialization of 'envir' field by %s of %s"
		run.test <- function(seed) {
			test_that(
				sprintf(template, deparse(substitute(seed)), function.name),
				{
					...
					expect_identical(adapter$envir, private$envir)
				}
			)
		}
...
ma.test$set(
	...
	function() {
		...
		# Find tests.
		test.names <- names(ma.test$private)
